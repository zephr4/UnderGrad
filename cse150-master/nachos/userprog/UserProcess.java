...
public class UserProcess {
    ...
    public int readVirtualMemory(int vaddr, byte[] data, int offset, int length) {
        ...
        bytesTransferred = bytesTransferred + amount;			
        }
    ...
    public int writeVirtualMemory(int vaddr, byte[] data, int offset, int length) {
        ...
        TranslationEntry TableEntry = pageTable[virtualPage];
        if (TableEntry.readOnly || TableEntry.valid == false) {
            break;
        }
        ...
        vaddr = vaddr + amount; 	
        length = length - amount;	
        bytesTransferred = bytesTransferred + amount;	//amount of bytes successfully transferred
        }
	...
	private int exec(int file, int argc, int argv) {
    	...
    	int child_ID = child.processID;
    	childID.add(child_ID);
    ...
    private void exit(int statusCode) {
    	...
    	if(processTally.isEmpty())
    	{
    		Kernel.kernel.terminate();
    	}
    	// check naaaachos for this
    ...
    public void cleanUpProcess() {
    	...
    	processTally.remove(processID);
    	processExitTally.put(processID, this);
    }
    
    private int join(int pid, int statusCode)
    {
    	if(childID.contains(pid) != true)
    	{
    		return -1;
    	}
    	childID.remove(pid);
    	UserProcess child = processTally.get(pid);
    	if(child == null)
    	{
    		child = processExitTally.get(pid);
    		if(child == null)
    		{
    			return -1;
    		}
    	...
    	child.semaphore.P();
    	writeVirtualMemory(statusCode, Lib.bytesFromInt(child.status));
    ...
    private int unlink(int name) {
        ...
        for (i = 2; i < this.OpenFiles.length; i++) {                       // Checks each open file, if the process we want to unlink is opened 
            ...
            if (OpenFiles[i].getName().equals(filename) && OpenFiles[i] != null) {
